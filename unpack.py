...
if ext_tri_count != 0:
    ...
    for idx_tri in range(ext_tri_count):
        ...
        ext_tri_counts = file[idx_head[0,ext_tri[idx_tri]]+14]*256 + file[idx_head[0,ext_tri[idx_tri]]+15]
        ...
        #print(f"External trigger {idx_tri+1}. Trigger count: {ext_tri_counts} (exceed:{exceed}),Trigger time stamp: {ext_tri_source_stamp}(exceed:{ext_tri_stamp_exceed})")
...
for idx in tqdm(range(idx_g_board.shape[0])):
    ...
    if pack_pointer_board_channel_timeStamp_valid[board_id_idx_test,channel_id_idx_test,timeStamp_idx_test]:
        ...
        plt.ylabel(r'$Data$')
        plt.ylim(bottom=0)
        plt.title(rf'$Board id: {board_id[0,idx]},channel id: {sub_pack_channel_id_int[idx]},time stamp: {sub_pack_trigger_source_stamp[idx]}$')
