...
inte_range = np.array([-10,14])#area used to calculate integartion
bg_range = 10# use wavedatapoint[0:bg_range] to calculate background
...
if num_ext_tri != 0:
    ...
    # combine close trigger package
    filtered_ext_trig_exceed = np.zeros(num_ext_tri)*np.nan
    filtered_ext_trig_count = np.zeros(num_ext_tri)*np.nan
    filtered_ext_trig_stamp = np.zeros(num_ext_tri)*np.nan
    filtered_ext_trig_sstamp_exceed = np.zeros(num_ext_tri)*np.nan
    filtered_ext_trig_stamp[0] = ext_tri_source_stamp[0]
    filtered_ext_trig_count[0] = ext_tri_count[0]
    filtered_ext_trig_exceed[0] = exceed[0]
    filtered_ext_trig_sstamp_exceed[0] = ext_tri_stamp_exceed[0]
    for idx_tri in range(num_ext_tri):
        if np.nanmin(np.abs(ext_tri_source_stamp[idx_tri]-filtered_ext_trig_stamp)) >1000: 
            filtered_ext_trig_stamp[idx_tri] = ext_tri_source_stamp[idx_tri]
            filtered_ext_trig_count[idx_tri] = ext_tri_count[idx_tri]
            filtered_ext_trig_exceed[idx_tri] = exceed[idx_tri]
            filtered_ext_trig_sstamp_exceed[idx_tri] = ext_tri_stamp_exceed[idx_tri]
    filtered_ext_trig_stamp = filtered_ext_trig_stamp[~np.isnan(filtered_ext_trig_stamp)]
    filtered_ext_trig_count = filtered_ext_trig_count[~np.isnan(filtered_ext_trig_count)]
    filtered_ext_trig_exceed = filtered_ext_trig_exceed[~np.isnan(filtered_ext_trig_exceed)]
    filtered_ext_trig_sstamp_exceed = filtered_ext_trig_sstamp_exceed[~np.isnan(filtered_ext_trig_sstamp_exceed)]
    ...
    filtered_num_ext_tri = filtered_ext_trig_stamp.shape[0]

    # conbine close datapackage trigger count
    related_trigger_idx = np.zeros(num_ext_tri)*np.nan
    for idx_this_teigger_count,this_trigger_count in enumerate(np.unique(sub_pack_trigger_source_count)):
        pack_idxs = np.where(sub_pack_trigger_source_count == this_trigger_count)[0]
        if not if_data_package[pack_idxs].all(): continue
        ave_time_stamp = np.mean(sub_pack_trigger_source_stamp[pack_idxs][if_data_package[pack_idxs]])
        idx_close_ext_trigger = np.argmin(np.abs(ave_time_stamp*25 - filtered_ext_trig_stamp*2.5))
        related_trigger_idx[idx_this_teigger_count] = idx_close_ext_trigger
        #print(f'{this_trigger_count}->{filtered_ext_trig_count[idx_close_ext_trigger]}')
else: print("No external trigger package found")
#-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
#find time difference with a external trigger
for test_ext_tri_idx in range (filtered_num_ext_tri):
    #test_ext_tri_idx = 0
    print(f'{test_ext_tri_idx+1}:external trigger count:{filtered_ext_trig_count[test_ext_tri_idx]}')
    related_data_pack_count = np.unique(sub_pack_trigger_source_count)[np.where(related_trigger_idx == test_ext_tri_idx)]
    print(f"packs with these trigger count considered related with this external trigger:{related_data_pack_count}")
    difference = np.array([])
    for this_idx,this_count in enumerate(related_data_pack_count):
        pack_idxs = np.where(sub_pack_trigger_source_count == this_count)[0]
        difference = np.append(difference, filtered_ext_trig_stamp[test_ext_tri_idx] * 2.5 - sub_pack_trigger_source_stamp[pack_idxs][if_data_package[pack_idxs]] * 25)
    print(difference)
    print(np.diff(np.unique(difference)))
"""
