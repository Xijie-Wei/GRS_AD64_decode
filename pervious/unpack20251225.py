...
#load file 
file_name = "RAW_data_20251218_152158.bin"
...
print(f"Package at {np.where(np.logical_not(if_data_package))[0]} are external trigger package")
# channel number
...
ext_tri = np.zeros(10).astype(np.int32)
ext_tri_count = 0
    ...
    for idx_pack in range(existed_pack_id.shape[0]):
        for idx_repeat in range(genral_pack_pointer.shape[2]):
            ...
            if not if_data_package[file_pointer]: 
                ext_tri[ext_tri_count] = file_pointer
                ext_tri_count += 1
                continue
            #print(file_pointer)
...
if ext_tri_count != 0:
    print(f"{ext_tri_count} external trigger package found")
    for idx_tri in range(ext_tri_count):
        #print(file[idx_head[0,ext_tri[idx_tri]]:idx_head[0,ext_tri[idx_tri]]+33])
...
"""            
board_id_idx_test = np.where(existed_board_id==2)[0]
channel_id_idx_test = np.where(existed_channel_id==47)[0]
#print(np.where(pack_pointer_board_channel_timeStamp_valid[board_id_idx_test,channel_id_idx_test,:]))
timeStamp_idx_test = np.where(pack_pointer_board_channel_timeStamp_valid[board_id_idx_test,channel_id_idx_test,:])[1][0]#np.where(existed_time_stamp=='0xf8d5610')[0]
...
if pack_pointer_board_channel_timeStamp_valid[board_id_idx_test,channel_id_idx_test,timeStamp_idx_test]:
    ...
    plt.ylabel(r'$Data$')
    plt.ylim(bottom=0)
    plt.title(rf'$Board id: {board_id[0,idx][0]},channel id: {sub_pack_channel_id_int[idx][0]},time stamp: {sub_pack_trigger_source_stamp[idx[0]]}$')
    plt.savefig("output")
    idx_head_test = idx_head[0,idx][0]
